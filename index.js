...
const { processCommand, processCommandStream } = require('./backend/router')
...
async function speak(text, language = 'auto') {
  const audio = await textToSpeech(text, language)
  await playAudio(audio)
}

// TTS por frases: la primera frase puede empezar a hablar mientras el LLM
// todavía está generando el resto. Cada frase se sintetiza por separado.
class SentenceTTSQueue {
  ...
  add(text, language = 'auto') {
    const clean = String(text || '').trim()
    if (!clean) return
    // La síntesis empieza inmediatamente; la reproducción mantiene el orden.
    const audioPromise = textToSpeech(clean, language)
    this.playQueue.push(audioPromise)
    this.drain()
  }
  ...
  async drain() {
    ...
    try {
      while (this.playQueue.length) {
        const audio = await this.playQueue.shift()
  ...
  async wait() {
    while (this.playQueue.length || this.playing) {
...
function splitCompleteSentences(buffer) {
  const out = []
  let rest = buffer
  // También permite frases cortas terminadas por ? o !
  const re = /^([\s\S]*?[.!?]+)(?:\s+|$)/
  while (true) {
  ...
  try {
    ...
    const onDelta = (delta, meta = {}) => {
      ...
      for (const sentence of split.sentences) {
        // Si el stream conoce el idioma, úsalo; si no, TTS lo detecta.
        ttsQueue.add(sentence, meta.language || 'auto')
      }
    }

    finalResult = await processCommandStream(text, onDelta)

    // Lo que quede sin puntuación al final también se reproduce.
    if (sentenceBuffer.trim()) {
      ttsQueue.add(sentenceBuffer.trim(), finalResult?.language || 'auto')
    }

    console.log(`[JARVIS] Respuesta: "${finalResult?.response || finalResult?.reply || ''}"`)
    await ttsQueue.wait()
  } catch (err) {
    setPaused(false)
...
function startTimerAnnouncements() {
  timerEvents.on('done', async ({ label }) => {
    ...
    try {
      await speak(message, process.env.TTS_LANGUAGE || 'en')
    } catch (err) {
